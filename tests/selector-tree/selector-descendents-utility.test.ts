...
describe('Selector Utility: Descendents', () => {
  ...
  it('Returns returns same ** node when not found', () => {
    ...
    addSelectorToTree(tree, ['someOtherPath', '**', 'someLeaf'], selectorFn1)
  ...
  it('handles numbers as keys as well', () => {
    const selectorFn1 = () => {}
    let tree: SelectorTreeBranch = { propName: 'root' }
    addSelectorToTree(tree, ['intermediary', 1], selectorFn1)
    const aMatch = getRefDescedents(tree.children!.intermediary, 1)
