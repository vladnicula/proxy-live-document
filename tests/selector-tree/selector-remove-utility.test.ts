...
describe('Selector Utility: Remove', () => {
  it('removes selector sub from tree', () => {
    ...
    expect(tree.children?.intermediary.children?.leaf2.subs).toContain(
      selectorFn2,
    )

    removeSelectorFromTree(refLeaf2, selectorFn2)
  ...
  it('silently fails if removing does not find item', () => {
    ...
    removeSelectorFromTree(refLeaf1, notSelectorFn)
