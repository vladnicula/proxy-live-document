...
describe('Selector Utility: Add', () => {
  it('creates first selector node in selector tree', () => {
    ...
    addSelectorToTree(tree, ['intermediary', 'leaf'], selectorFn)
    ...
    expect(tree.children?.intermediary.children?.leaf).not.toHaveProperty(
      'children',
    )
    expect(tree.children?.intermediary.children?.leaf.subs?.[0]).toBe(
      selectorFn,
    )
  })
  ...
  it('creates second selector branching off common path', () => {
    ...
    addSelectorToTree(tree, ['intermediary', 'leaf1'], selectorFn1)
    ...
    expect(tree.children?.intermediary.children?.leaf1.subs).toContain(
      selectorFn1,
    )

    expect(tree.children?.intermediary.children).toHaveProperty(
      'someSubObject',
    )
    expect(tree.children?.intermediary.children?.someSubObject).toHaveProperty(
      'children',
    )
    expect(
      tree.children?.intermediary.children?.someSubObject.children,
    ).toHaveProperty('leaf2')
    expect(
      tree.children?.intermediary.children?.someSubObject.children?.leaf2.subs,
    ).toContain(selectorFn2)
  })
  ...
  it('creates second selector in same path', () => {
    ...
    addSelectorToTree(tree, ['intermediary', 'leaf1'], selectorFn2)

    expect(tree.children?.intermediary).toHaveProperty('children')
    expect(tree.children?.intermediary.children).toHaveProperty('leaf1')
    expect(tree.children?.intermediary.children?.leaf1).not.toHaveProperty(
      'children',
    )
    expect(tree.children?.intermediary.children?.leaf1.subs).toContain(
      selectorFn1,
