...
describe('selector reaction to ancestors', () => {
  ...
  it('selector runs when key is deleted and selectors listens to ** ', () => {
    ...
    select(
      myState,
      ['nodesById/2/styles/padding'],
      () => {
        callbackSpy(
          'nodesById/2/styles selector running with ancestor reactivity',
        )
      },
  ...
  it('selector runs when key is deleted and selectors listens to ** ', () => {
    ...
    select(
      myState,
      ['nodesById/1/styles/padding'],
      () => {
        callbackSpy(
          'nodesById/1/styles selector running with ancestor reactivity',
        )
      },
      { reactToAncestorChanges: true },
    )
