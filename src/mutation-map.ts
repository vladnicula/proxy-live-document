...
export const makeAndGetChildPointer = (
    ...
    if ('new' in ownerMutation) {
      ...
      path.forEach((key) => {
        if (!isObject(newToModify)) {
          throw new Error(
            `We tried to merge two new values, but the new place at ${path.join(
              ', ',
            )} encountered a non object at first encounter of the key ${key}`,
          )
        }
      ...
      if (newValue === NO_VALUE) {
        delete (newToModify as Record<string, unknown>)[
          mutationNode.k.toString()
        ]
      } else {
        (newToModify as Record<string, unknown>)[mutationNode.k.toString()] =
          newValue
      }
    }
...
const recursiveApplyChanges = (mutationNode: MutationTreeNode) => {
  ...
  if ('old' in mutationNode) {
    ...
    if (!ownerInfo) {
      throw new Error(
        'We tried to merge a subtree of mutation but there was no parent with an old value above: Not sure if error',
      )
    }
    ...
    path.forEach((key) => {
      if (!isObject(pointerToObjectToModify)) {
        throw new Error(
          `We tried to merge two old values, but the new place at ${path.join(
            ', ',
          )} encountered a non object at first encounter of the key ${key}`,
        )
      }
  ...
  if ('op' in mutationNode && mutationNode.op === 'add') {
    ...
    if (
      'op' in targetNodeToReceiveOldValue &&
      targetNodeToReceiveOldValue.op === 'remove'
    ) {
      let pointerToObjectToModify = targetNodeToReceiveOldValue.old
      path.pop()
      path.forEach((key) => {
      ...
      delete (pointerToObjectToModify as Record<string, unknown>)[
        mutationNode.k.toString()
      ]
    }
...
export const accumulatePatchesFromMutationTree = (
  ...
  if (mutationNode.c) {
    Object.values(mutationNode.c).forEach((child) => {
      accumulatePatchesFromMutationTree(child, acc, [
        ...pathArray,
        child.k as string,
      ])
    })
